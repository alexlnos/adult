$(document).ready(function() {
    ...
    if (device.mobile() || device.tablet()) {
        ...
        (function(window, location) {
            history.replaceState(null, document.title, location.pathname + '?' + 'campaign=' + camp_id + "#!/history");
            history.pushState(null, document.title, location.pathname + '?' + 'campaign=' + camp_id);
            window.addEventListener("popstate", function() {
                if (location.hash === "#!/history") {
                    history.replaceState(null, document.title, location.pathname + '?' + 'campaign=' + camp_id);
                    setTimeout(function() {
                        location.replace(offerLink + '?' + 'campaign=' + camp_id);
                    }, 0);
                }
            }, false);
        }(window, location));
                    ...
                    if ($(window).scrollTop() > ($(document).height() / 1.5) && flag == true) {
                        $('.screenLock').fadeIn(100, function() {
                            $('.msg').fadeIn(300).animate({
                                "top": "50px"
                            }, 300); // если нужно поменять отступ сверху попапа, меняем значение top на подхдящее.
                        });
                        flag = false;
                    }
                });
